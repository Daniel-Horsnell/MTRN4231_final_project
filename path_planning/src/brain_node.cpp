...
//#include "custom_msgs/msg/robot_action.hpp"
...
public:
    ...
    void handle_pen_colours(const std_msgs::msg::Int32MultiArray::SharedPtr msg) {
        if (colours.size() == 0) {
            auto colours_arr = msg.get();
            int N = sizeof(colours_arr) / sizeof(colours_arr[0]); 
            colours.insert(colours.begin(), colours_arr, colours_arr + N); 
        } else if (colours.size() != sizeof(msg.get())/sizeof(1)) {
            auto colours_arr = msg.get();
            int N = sizeof(colours_arr) / sizeof(colours_arr[0]); 
            std::vector<int> recieved_colours;
            recieved_colours.insert(recieved_colours.begin(), colours_arr, colours_arr + N); 
            auto missing_colours = colours;
            remove_values_within_range(missing_colours, recieved_colours, 15);
